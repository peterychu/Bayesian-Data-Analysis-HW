...
$$
L(\mu) = \prod_{i = 1}^{n} \frac{{\lambda ^{y_i}} * e^{- \lambda}}{y_i!} \propto \lambda ^ {\Sigma y_i} * e^{- \lambda n}
$$
## 1b. 
...
$$
l(\lambda) = log(\prod_{i = 1}^{n} \frac{{\lambda ^{y_i}} * e^{- \lambda}}{y_i!}) = \Sigma_{i=1}^n log(\lambda^{y_i} * \frac{1}{y_i!} * e^{- \lambda}) =
$$

$$
\Sigma_{i = 1}^n(log(\lambda^{y_i}) - log(y_i) + log(e^{- \lambda})) = \Sigma_{i=1}^n ( y_i log(\lambda) - log(y_i!) -\lambda) = \Sigma y_i log(\lambda) - \Sigma log(y_i!) -n \lambda
$$
MLE

$$
\hat{\lambda}_{mle} = \frac{d}{d \lambda} l(\lambda) = 0 \Rightarrow \frac{d}{d \lambda} \Sigma y_i log(\lambda) - \Sigma log(y_i!) -n \lambda = \frac{\Sigma y_i}{\lambda} - n = 0 \Rightarrow \hat{\lambda}_{mle} = \frac{\Sigma y_i}{n}
$$
...
$\frac{\nu_i}{1000}$ is the number of words per chapter divided by 1000. In this model $\lambda$ is the number if times "fire" appears per 1000 words. 
...
The mle calculated here is 0.62 which means that a $\lambda$ value of 0.62 makes the data most likely. It means that for every 1000 pages the word _fire_ will come up 0.62 times.
...
f2 = function(lambda){
  sum(fire_counts) * log(lambda) - sum(log(factorial(fire_counts))) - sum(chapter_lengths) * (lambda / 1000)
...
It might be unreasonable to assume that the rate of _fire_ usage is the same in all chapters because different chapters will be about different topics. For example, the introduction can be vastly different from the ending which could lead to different rates of _fire_. In addition certain chapters could have more dialogue which could lead to a lower rate of _fire_. 
...
The simulated graph appears to  have more counts more frequently. In addition, the observed graph appears to have values when fire > 10, but on our simulated graphh there are none. It is possible that the observed graph had these outliers which are not in the simulated one, but it could also be just by chance this time that it didn't.
...
Write out the the data generating process for the above model.  REDO REDO RDDO

The data generating process for the above model would include randomly sampling from a Gamma distribution with known $\alpha$ and $\beta$ 1000 times and seeing the results. 
...
In our case the variance is slightly greater than the mean so our distribution is overdispsersed. This makes sense as we did take random samples and there could have been a single relatively small outlier which slightly skews the variance to be greater than the mean. 
...
The known variables with variance > 0 are $Y_i$. The known variables with variance = 0 are $y_i$, $\alpha$, $\beta$, and $n$. The unknown variable with varaince > 0 is $\lambda$. There are no unknown variables with variance = 0.
